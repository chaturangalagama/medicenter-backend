...
@Service
public class DefaultPatientReferralDownstream implements PatientReferralDownstream {

    private static final Logger logger = LoggerFactory.getLogger(DefaultPatientReferralDownstream.class);
    private PatientReferralService patientReferralService;

    public DefaultPatientReferralDownstream(PatientReferralService patientReferralService) {
    ...
    public ResponseEntity<ApiResponse> searchById(String referralId) {
        try {
            ...
            return httpApiResponse(new HttpApiResponse(MedicalReferenceMapper.mapPatientReferralToEntity(referral)));
    ...
    public ResponseEntity<ApiResponse> create(com.ilt.cms.api.entity.consultation.PatientReferral patientReferral) {
        try {
            PatientReferral createdReferral = patientReferralService.createReferral(MedicalReferenceMapper.mapPatientReferralToCore(patientReferral));
            logger.info("PatientReferral created [id]:[" + createdReferral.getId() + "]");
            return httpApiResponse(new HttpApiResponse(MedicalReferenceMapper.mapPatientReferralToEntity(createdReferral)));
        } catch (CMSException e) {
    ...
    public ResponseEntity<ApiResponse> update(String referralId, com.ilt.cms.api.entity.consultation.PatientReferral patientReferral) {
        try {
            PatientReferral referral = patientReferralService.updateReferral(referralId, MedicalReferenceMapper.mapPatientReferralToCore(patientReferral));
